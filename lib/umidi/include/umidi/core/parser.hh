...
/// @file parser.hh
/// @brief Optimized MIDI 1.0 byte stream to UMP32 parser.
#pragma once
...
/// @brief Converts MIDI 1.0 byte stream to UMP32.
///
/// Optimizations:
/// - Incremental UMP word construction during parse
/// - Single status byte classification
/// - Minimal branching with early detection of 2-byte messages
///
/// @par Example:
/// @code
/// Parser parser;
/// UMP32 ump;
/// for (uint8_t byte : midi_bytes) {
...
public:
    ...
    /// Parse a single byte and return UMP32 if message is complete
    /// @param byte Input byte
    /// @param sample_pos Sample position for timing (passed through)
    /// @param out Output UMP32 (valid only if returns true)
    ...
    /// If no status byte is received, uses previous running status
    ...
    /// Get current running status (0 if none)
...
class Serializer {
public:
    /// Serialize UMP32 to MIDI 1.0 bytes
    /// @param ump Input UMP
    /// @param out Output buffer (must be at least 3 bytes)
    /// @return Number of bytes written (0, 1, 2, or 3)
    ...
    /// @param ump Input UMP
    /// @param out Output buffer (must be at least 3 bytes)
    /// @param running_status Current running status (updated on output)
