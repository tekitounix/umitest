...
/// Must fit in a single flash sector
struct BootConfig {
...
/// STM32F4 (e.g., STM32F411, 512KB flash)
/// Sector sizes: 16K x 4, 64K x 1, 128K x 3
...
/// STM32H7 (e.g., STM32H743, 2MB flash)
...
/// ESP32 (via partition table)
/// Note: ESP32 uses its own partition system, these are typical OTA sizes
...
/// RP2040 (Raspberry Pi Pico, 2MB flash)
/// Uniform 4KB sectors
...
/// Flash interface (platform must implement)
struct FlashInterface {
    /// Erase sectors containing address range
    FlashResult (*erase)(uint32_t address, size_t size);

    /// Write data (must be aligned to write_size)
...
/// Manages A/B partition switching and boot configuration
template <const PlatformConfig* Config>
class BootloaderInterface {
public:
    /// Initialize with flash interface
    ...
    /// Prepare slot for writing (erase)
    ...
    /// Mark slot as valid after successful verification
    ...
    /// Set slot as pending (update received but not committed)
    void mark_slot_pending(Slot slot) noexcept {
        set_slot_state(slot, SlotState::PENDING);
    }

    /// Commit update (switch active slot)
    ...
    /// Call this from application after successful startup
    ...
    /// Increment boot count (called by bootloader on each boot)
    ...
    /// Get boot configuration (read-only)
