...
// umihal concept satisfaction tests
...
#include <functional>
#include <span>
#include <umihal/arch.hh>
#include <umihal/audio.hh>
#include <umihal/board.hh>
#include <umihal/codec.hh>
#include <umihal/concept/clock.hh>
#include <umihal/concept/codec.hh>
#include <umihal/concept/platform.hh>
#include <umihal/concept/transport.hh>
#include <umihal/concept/uart.hh>
#include <umihal/fault.hh>
#include <umihal/gpio.hh>
#include <umihal/i2c.hh>
#include <umihal/i2s.hh>
#include <umihal/interrupt.hh>
#include <umihal/result.hh>
#include <umihal/timer.hh>
...
// --- GpioPin ---
struct Pin {
    Result<void> set_direction(umi::hal::gpio::Direction) { return {}; }
    Result<void> set_pull(umi::hal::gpio::Pull) { return {}; }
    Result<void> write(umi::hal::gpio::State) { return {}; }
    Result<umi::hal::gpio::State> read() { return umi::hal::gpio::State::LOW; }
    Result<void> set_output_type(umi::hal::gpio::OutputType) { return {}; }
    Result<void> set_interrupt(umi::hal::gpio::Trigger, umi::hal::gpio::InterruptCallback) { return {}; }
...
// --- InterruptController ---
struct Intc {
    Result<void> register_handler(std::uint32_t, umi::hal::interrupt::Handler) { return {}; }
    Result<void> enable_irq(std::uint32_t) { return {}; }
    Result<void> disable_irq(std::uint32_t) { return {}; }
    Result<void> set_priority(std::uint32_t, umi::hal::interrupt::Priority) { return {}; }
    void enable_global() {}
    void disable_global() {}
...
struct Tim {
    ...
    Result<void> set_mode(umi::hal::timer::Mode) { return {}; }
    Result<void> set_callback(umi::hal::timer::Callback) { return {}; }
    Result<std::uint32_t> get_counter() { return std::uint32_t{0}; }
    Result<void> set_counter(std::uint32_t) { return {}; }
    Result<void> set_clock_source(umi::hal::timer::ClockSource) { return {}; }
    Result<void> set_prescaler(std::uint32_t) { return {}; }
    Result<void> set_direction(umi::hal::timer::Direction) { return {}; }
    Result<void> enable_capture(umi::hal::timer::CaptureEvent, umi::hal::timer::CaptureCallback) { return {}; }
...
// --- Uart ---
struct UartDev {
    Result<void> init(const umi::hal::uart::Config&) { return {}; }
    ...
    Result<void> write_async(std::span<const std::uint8_t>, umi::hal::uart::TxCallback) { return {}; }
    Result<void> read_async(std::span<std::uint8_t>, umi::hal::uart::RxCallback) { return {}; }
...
// --- I2cMaster ---
struct I2c {
    Result<void> init(umi::hal::i2c::Speed) { return {}; }
    Result<void> deinit() { return {}; }
    ...
    Result<void> write_async(std::uint16_t, std::span<const std::uint8_t>, umi::hal::i2c::TransferCallback) {
        return {};
    }
    Result<void> read_async(std::uint16_t, std::span<std::uint8_t>, umi::hal::i2c::TransferCallback) { return {}; }
    Result<void> write_read_async(std::uint16_t,
                                  std::span<const std::uint8_t>,
                                  std::span<std::uint8_t>,
                                  umi::hal::i2c::TransferCallback) {
        return {};
    }
...
// --- I2sMaster ---
struct I2s {
    Result<void> init(const umi::hal::i2s::Config&) { return {}; }
    Result<void> deinit() { return {}; }
    ...
    Result<void> transmit_async(std::span<const std::uint16_t>, umi::hal::i2s::TransferCallback) { return {}; }
    Result<void> receive_async(std::span<std::uint16_t>, umi::hal::i2s::TransferCallback) { return {}; }
    Result<void>
    transmit_receive_async(std::span<const std::uint16_t>, std::span<std::uint16_t>, umi::hal::i2s::TransferCallback) {
        return {};
    }
    Result<void> start_continuous_transmit(std::span<const std::uint16_t>, umi::hal::BufferCallback) { return {}; }
    Result<void> start_continuous_receive(std::span<std::uint16_t>, umi::hal::BufferCallback) { return {}; }
...
// --- AudioDevice ---
struct AudioDev {
    Result<void> configure(const umi::hal::audio::Config&) { return {}; }
    umi::hal::audio::Config get_config() { return {}; }
    bool is_config_supported(const umi::hal::audio::Config&) { return true; }
    Result<void> start() { return {}; }
    Result<void> stop() { return {}; }
    Result<void> pause() { return {}; }
    Result<void> resume() { return {}; }
    umi::hal::audio::State get_state() { return umi::hal::audio::State::STOPPED; }
    ...
    // CallbackAudioDevice
    Result<void> set_callback(umi::hal::audio::Callback<std::int16_t>) { return {}; }
    // BlockingAudioDevice
...
// --- I2cTransport ---
struct TransportI2c {
    Result<void> write(uint8_t, uint8_t, std::span<const uint8_t>) { return {}; }
    Result<void> read(uint8_t, uint8_t, std::span<uint8_t>) { return {}; }
};

// --- SpiTransport ---
struct TransportSpi {
    Result<void> transfer(std::span<const uint8_t>, std::span<uint8_t>) { return {}; }
    void select() {}
    void deselect() {}
};
...
struct Clock {
    ...
    static uint32_t get_frequency() { return 168'000'000; }
};

// --- ClockTree ---
struct ClockTreeImpl {
    static void init() {}
    static uint32_t system_clock() { return 168'000'000; }
    static uint32_t ahb_clock() { return 168'000'000; }
    static uint32_t apb1_clock() { return 42'000'000; }
};
...
struct PlatformStub {
    using Output = PlatformOutput;
    static void init() { Output::init(); }
};

struct PlatformWithTimerStub {
    using Output = PlatformOutput;
    struct Timer {
        static void enable() {}
...
static_assert(umi::hal::CacheOps<stub::Cache>);
static_assert(umi::hal::FpuOps<stub::Fpu>);
static_assert(umi::hal::ContextSwitch<stub::Context>);
static_assert(umi::hal::ArchTraits<stub::Arch>);
static_assert(umi::hal::BoardSpec<stub::Board>);
static_assert(umi::hal::McuInit<stub::Mcu>);
static_assert(umi::hal::AudioCodec<stub::Codec>);
static_assert(umi::hal::FaultReport<stub::Fault>);
static_assert(umi::hal::GpioPin<stub::Pin>);
static_assert(umi::hal::GpioPort<stub::Port>);
static_assert(umi::hal::InterruptController<stub::Intc>);
static_assert(umi::hal::Timer<stub::Tim>);
static_assert(umi::hal::DelayTimer<stub::Delay>);
static_assert(umi::hal::PwmTimer<stub::Pwm>);
static_assert(umi::hal::Uart<stub::UartDev>);
static_assert(umi::hal::I2cMaster<stub::I2c>);
static_assert(umi::hal::I2sMaster<stub::I2s>);
static_assert(umi::hal::audio::AudioDevice<stub::AudioDev>);
static_assert(umi::hal::audio::CallbackAudioDevice<stub::AudioDev, std::int16_t>);
static_assert(umi::hal::audio::BlockingAudioDevice<stub::AudioDev, std::int16_t>);

// New concept assertions (Phase 1)
static_assert(umi::hal::UartBasic<stub::UartBasicDev>);
static_assert(umi::hal::UartBasic<stub::UartDev>);
static_assert(umi::hal::I2cTransport<stub::TransportI2c>);
static_assert(umi::hal::SpiTransport<stub::TransportSpi>);
static_assert(umi::hal::ClockSource<stub::Clock>);
static_assert(umi::hal::ClockTree<stub::ClockTreeImpl>);
static_assert(umi::hal::OutputDevice<stub::PlatformOutput>);
static_assert(umi::hal::Platform<stub::PlatformStub>);
static_assert(umi::hal::PlatformWithTimer<stub::PlatformWithTimerStub>);
static_assert(umi::hal::CodecBasic<stub::CodecBasicDev>);
static_assert(umi::hal::CodecBasic<stub::CodecWithVol>);
static_assert(umi::hal::CodecWithVolume<stub::CodecWithVol>);
static_assert(umi::hal::CodecBasic<stub::Codec>);
static_assert(umi::hal::CodecWithVolume<stub::Codec>);
static_assert(umi::hal::AudioCodec<stub::Codec>);
...
static void test_result_basics(Suite& s) {
    ...
    {
        umi::hal::Result<int> ok = 42;
        s.check(ok.has_value(), "Result<int> has value");
        s.check_eq(*ok, 42);

        umi::hal::Result<void> ok_void{};
        s.check(ok_void.has_value(), "Result<void> has value");

        umi::hal::Result<int> err = std::unexpected(umi::hal::ErrorCode::TIMEOUT);
        s.check(!err.has_value(), "Error result has no value");
        s.check_eq(static_cast<int>(err.error()), static_cast<int>(umi::hal::ErrorCode::TIMEOUT));
    }
}
...
static void test_gpio_pin(Suite& s) {
    ...
    {
        stub::Pin pin;
        s.check(pin.set_direction(umi::hal::gpio::Direction::OUTPUT).has_value(), "set_direction");
        s.check(pin.write(umi::hal::gpio::State::HIGH).has_value(), "write HIGH");
        auto state = pin.read();
...
static void test_critical_section(Suite& s) {
    s.section("CriticalSection RAII");
    {
        stub::Intc intc;
        {
            umi::hal::CriticalSection cs(intc);
        }
...
static void test_i2c_master(Suite& s) {
    ...
    {
        stub::I2c i2c;
        s.check(i2c.init(umi::hal::i2c::Speed::FAST).has_value(), "init");
        std::uint8_t buf[4]{};
...
static void test_new_error_codes(Suite& s) {
    s.section("New ErrorCode values (Phase 2)");
    {
        umi::hal::Result<int> nack = std::unexpected(umi::hal::ErrorCode::NACK);
        s.check(!nack.has_value(), "NACK is error");
        s.check_eq(static_cast<int>(nack.error()), static_cast<int>(umi::hal::ErrorCode::NACK));

        umi::hal::Result<int> bus = std::unexpected(umi::hal::ErrorCode::BUS_ERROR);
        s.check(!bus.has_value(), "BUS_ERROR is error");

        umi::hal::Result<int> overrun = std::unexpected(umi::hal::ErrorCode::OVERRUN);
        s.check(!overrun.has_value(), "OVERRUN is error");

        umi::hal::Result<int> invalid = std::unexpected(umi::hal::ErrorCode::INVALID_CONFIG);
        s.check(!invalid.has_value(), "INVALID_CONFIG is error");
    }
}

static void test_platform_concept(Suite& s) {
    s.section("Platform concept satisfaction");
    {
        stub::PlatformStub::init();
        stub::PlatformStub::Output::putc('A');
        s.check(true, "Platform stub callable");
    }
}

static void test_transport_concepts(Suite& s) {
    s.section("Transport concept satisfaction");
    {
        stub::TransportI2c i2c;
        uint8_t buf[4]{};
        auto r = i2c.write(0x4A, 0x00, std::span{buf});
        s.check(r.has_value(), "I2C write ok");
        r = i2c.read(0x4A, 0x00, std::span{buf});
        s.check(r.has_value(), "I2C read ok");

        stub::TransportSpi spi;
        spi.select();
        auto sr = spi.transfer(std::span<const uint8_t>{buf}, std::span{buf});
        s.check(sr.has_value(), "SPI transfer ok");
        spi.deselect();
    }
}

static void test_clock_concepts(Suite& s) {
    s.section("Clock concept satisfaction");
    {
        stub::Clock::enable();
        s.check(stub::Clock::is_ready(), "Clock ready");
        s.check_eq(static_cast<int>(stub::Clock::get_frequency() / 1'000'000), 168);

        stub::ClockTreeImpl::init();
        s.check_eq(static_cast<int>(stub::ClockTreeImpl::system_clock() / 1'000'000), 168);
        s.check_eq(static_cast<int>(stub::ClockTreeImpl::apb1_clock() / 1'000'000), 42);
    }
}

static void test_codec_hierarchy(Suite& s) {
    s.section("Codec concept hierarchy");
    {
        stub::CodecBasicDev basic;
        s.check(basic.init(), "CodecBasic init");

        stub::CodecWithVol vol;
        s.check(vol.init(), "CodecWithVolume init");
        vol.set_volume(-6);
        s.check(true, "CodecWithVolume set_volume callable");

        stub::Codec full;
        s.check(full.init(), "AudioCodec init");
        full.power_on();
        full.set_volume(-12);
        full.mute(true);
        full.power_off();
        s.check(true, "AudioCodec all methods callable");
    }
}

int main() {
    Suite s("umihal_concepts");
    ...
    test_i2s_master(s);
    test_audio_device(s);
    test_new_error_codes(s);
    test_platform_concept(s);
    test_transport_concepts(s);
    test_clock_concepts(s);
    test_codec_hierarchy(s);
