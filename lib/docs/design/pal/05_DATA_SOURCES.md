...
各 PAL カテゴリ (C1–C14) の定義を、対象プラットフォームごとに **どこから取得・抽出できるか**、**どのようなツールが必要か**、**何が手動作業になるか** を網羅的に整理する。
...
|---------|-------|--------------|-------------|---------|
| **C1 コアペリフェラル (MMIO)** | ○ D2+D9 | ○ D6+D9 | △ D7+D9 | ○ D8+D9 |
| **C2 コアイントリンシクス (非MMIO)** | ○ D2+D9 | ○ D6+D9 | △ D7+D9 | ○ D8+D9 |
| **C3 コアシステム** | ◎ D3/D2 | ◎ D6 | ◎ D7 | ◎ D8/D3 |
| **C4 ベクター** | ◎ D2 | ◎ D6 | △ D7 | ◎ D8 |
| **C5 メモリマップ** | ◎ D3/D1 | ◎ D6 | ○ D7 | ◎ D8/D3 |
| **C6 ペリフェラルレジスタ** | ○ D1+C1 | ◎ D6 | △ D7 | ◎ D8 |
| **C7 GPIO MUX** | ◎ D5/D4 | ◎ D6 | ◎ D7 | ○ D8 |
| **C8 クロックツリー** | △ D4/D10 | △ D6/D10 | △ D7/D10 | △ D8/D10 |
| **C9 DMA マッピング** | △ D4/D10 | ◎ D6 | △ D7/D10 | ○ D8 |
| **C10 電力管理** | ✎ D10 | ✎ D10 | ✎ D10 | ✎ D10 |
| **C11 セキュリティ** | ✎ D10 | ✎ D10 | ✎ D10 | ✎ D10 |
| **C12 デバイスメタ** | ◎ D3/D4 | ◎ D6 | ◎ D7 | ◎ D8/D3 |
| **C13 リンカ/スタートアップ** | ◎ D3/D2 | ○ D6 | △ D7 | ○ D8 |
| **C14 デバッグ/トレース** | ○ D3/D9 | ○ D6 | △ D7 | ○ D3/D9 |

---

## 5. カテゴリ別データソース詳細

### 5.1 C1: コアペリフェラルレジスタ — MMIO (NVIC, SCB, SysTick, DWT, ITM, MPU, FPU)
...
### 5.2 C2: コア特殊レジスタ・命令イントリンシクス (非 MMIO)

メモリマップされない CPU コア固有のレジスタおよび命令。
専用命令 (ARM: MSR/MRS、RISC-V: csrr/csrw、Xtensa: rsr/wsr) でアクセス。

#### 全プラットフォーム共通

| データ | STM32 ソース | RP ソース | ESP ソース | iMXRT ソース | 自動化 |
|--------|------------|---------|----------|------------|--------|
| ARM 特殊レジスタ (PRIMASK, BASEPRI, CONTROL, PSP/MSP) | D2 `cmsis_gcc.h` + D9 ARM Ref | D2/D6 | — | D2/D8 | ○ |
| ARM メモリバリア (DSB/ISB/DMB) | D2 `cmsis_gcc.h` | D2/D6 | — | D2 | ○ |
| ARM 排他アクセス (LDREX/STREX) | D2 `cmsis_gcc.h` | D2/D6 | — | D2 | ○ |
| ARM DSP/SIMD 命令 | D2 `cmsis_gcc.h` | — | — | D2 | ○ |
| ARM TrustZone 命令 (TT, BXNS) | D9 ARMv8-M Ref | D6 (RP2350 M33) | — | — | △ |
| ARM キャッシュ操作 | D2 `core_cm7.h` + D9 | — | — | D2 | △ |
| RISC-V CSR (mstatus, mie, mcause 等) | — | D6 Pico SDK + RISC-V ISA Manual | D7 ESP-IDF | — | △ |
| RISC-V アトミック命令 (LR/SC, AMO) | — | RISC-V ISA Manual (A 拡張) | D7 ESP-IDF | — | △ |
| Xtensa Special Registers (PS, CCOUNT, INTENABLE) | — | — | D7 ESP-IDF LL ヘッダ | — | △ |
| Xtensa MAC16/PIE SIMD | — | — | D7 ESP-IDF + Xtensa ISA Ref | — | ✎ |

**既存事例:**
- **CMSIS**: `cmsis_gcc.h` に全 ARM イントリンシクスが inline asm 関数として実装済み。PAL の最も信頼性の高いリファレンス。
- **Rust cortex-m クレート**: `asm!` マクロで同等の関数を Rust で実装。
- **UMI 既存**: `lib/umi/port/arch/cm4/arch/cortex_m4.hh` の `CM4` 構造体に部分的な実装あり (cpsid_i, dsb, isb, wfi, primask, psp, msp)。

**UMI 戦略:**
1. **Phase 1**: CMSIS `cmsis_gcc.h` (D2) を参考に、ARM Cortex-M 向けイントリンシクスを `pal/arch/arm/cortex_m/intrinsics.hh` として実装。既存 port 層の `CM4` をリファクタリング。
2. **Phase 2**: RISC-V CSR + アトミック命令を `pal/arch/riscv/intrinsics.hh` として追加。
3. **Phase 3**: Xtensa Special Registers + DSP を `pal/arch/xtensa/intrinsics.hh` として追加。

**必要ツール:**
- 手書きが主体 (アーキテクチャ仕様は固定であり、自動生成の利点が少ない)
- CMSIS `cmsis_gcc.h` との対応検証テスト (ビルトイン/inline asm の正確性確認)
...
### 5.6 C6: ペリフェラルレジスタ (GPIO, UART, SPI, I2C, Timer, ADC, DMA 等)
...
**UMI 戦略:**
1. PWR/PMU レジスタ定義は SVD から自動生成 (C6 の一部として)
...
**UMI 戦略:**
1. 暗号エンジン等のレジスタ定義は SVD から自動生成 (C6 の一部)
...
### 5.13 C13: リンカ / スタートアップ

#### STM32

| データ | 最適ソース | 自動化 |
|--------|----------|--------|
| メモリリージョン (FLASH/SRAM/CCM) | C5 メモリマップから派生 | ◎ |
| セクション配置 | CMSIS テンプレート | ◎ — テンプレート化可能 |
| スタック / ヒープサイズ | アプリケーション設定 | — (ユーザー指定) |
| ベクターテーブル初期配置 | C4 ベクターから派生 | ◎ |
...
**UMI 戦略:**
C5 メモリマップ + C4 ベクター定義から、Jinja2 テンプレートでリンカスクリプトとスタートアップコードを自動生成。
...
| カテゴリ | データ | 条件 |
|---------|--------|------|
| C3 | コアシステム定義 | CMSIS-Pack / SDK からの抽出 |
| C4 | 割り込みベクター | CMSIS ヘッダ / SVD からの抽出 |
| C5 | メモリマップ | CMSIS-Pack / SVD / SDK からの抽出 |
| C6 | ペリフェラルレジスタ (RP/iMXRT) | SVD 品質が良好なプラットフォーム |
| C7 | GPIO MUX (RP/ESP) | SDK / SVD に完全記述 |
| C12 | デバイスメタデータ | CMSIS-Pack / CubeMX DB からの抽出 |
| C13 | リンカ/スタートアップ (Cortex-M 標準) | C4+C5 からの派生生成 |

### 7.2 自動化 + パッチ/キュレーション必要

| カテゴリ | データ | 必要な追加作業 |
|---------|--------|-------------|
| C1 | コアペリフェラルレジスタ (MMIO) | アーキテクチャ数 (≤8) 分の手書き or AI 生成 |
| C2 | コアイントリンシクス (非MMIO) | CMSIS `cmsis_gcc.h` 参考に手書き + 既存 port 層リファクタリング |
| C6 | ペリフェラルレジスタ (STM32) | stm32-rs パッチ適用が前提 |
| C7 | GPIO MUX (STM32) | Open Pin Data パーサ開発 |
| C8 | クロックツリー (イネーブル/リセット) | SVD の RCC レジスタから自動抽出可能 |
| C9 | DMA マッピング (DMAMUX 搭載) | SVD + CubeMX DB からの複合抽出 |
| C14 | デバッグ/トレース | CMSIS-Pack CoreSight + コアペリフェラル |

### 7.3 手動キュレーション主体

| カテゴリ | データ | 理由 |
|---------|--------|------|
| C8 | クロックツリー (制約/周波数上限) | 複雑な依存関係、アプリケーション要件依存 |
| C9 | DMA マッピング (固定テーブル型) | RM テーブルからの転記が必要 |
| C10 | 電力管理 | スリープモード遷移、ウェイクアップレイテンシ等 |
| C11 | セキュリティ/保護 | OTP レイアウト、セキュアブートシーケンス等 |
| C13 | リンカ/スタートアップ (ESP/iMXRT 固有) | 独自ブートシーケンス |
