...
struct AudioContext {
    ...
    // ヘルパー関数
    const sample_t* input(size_t ch) const { return ch < num_inputs ? inputs[ch] : nullptr; }

    sample_t* output(size_t ch) const { return ch < num_outputs ? outputs[ch] : nullptr; }
};
...
class WasmAdapter {
  public:
    ...
    static void create() { sample_position() = 0; }
    ...
    static void set_param(uint32_t index, float value) { instance().set_param(index, value); }

    static float get_param(uint32_t index) { return instance().get_param(index); }

    static uint32_t get_param_count() { return ParamCount; }

    static const char* get_param_name(uint32_t i) { return i < ParamCount ? Params[i].name : ""; }

    static float get_param_min(uint32_t i) { return i < ParamCount ? Params[i].min : 0.0f; }

    static float get_param_max(uint32_t i) { return i < ParamCount ? Params[i].max : 1.0f; }

    static float get_param_default(uint32_t i) { return i < ParamCount ? Params[i].default_value : 0.0f; }

    static uint8_t get_param_curve(uint32_t i) { return i < ParamCount ? static_cast<uint8_t>(Params[i].curve) : 0; }

    static uint16_t get_param_id(uint32_t i) { return i < ParamCount ? Params[i].id : 0; }

    static const char* get_param_unit(uint32_t i) { return i < ParamCount ? Params[i].unit : ""; }
};
...
#define UMIM_EXPORT(ProcessorType, params_array)                                                         \
    using _UmiAdapter = umi::umim::WasmAdapter<ProcessorType, params_array.data(), params_array.size()>; \
                                                                                                         \
    extern "C" {                                                                                         \
    UMIM_EXPORT_ATTR void umi_create(void) {                                                             \
        _UmiAdapter::create();                                                                           \
    }                                                                                                    \
    ...
    UMIM_EXPORT_ATTR void umi_note_on(uint8_t n, uint8_t v) {                                            \
        _UmiAdapter::note_on(n, v);                                                                      \
    }                                                                                                    \
    UMIM_EXPORT_ATTR void umi_note_off(uint8_t n) {                                                      \
        _UmiAdapter::note_off(n);                                                                        \
    }                                                                                                    \
    UMIM_EXPORT_ATTR void umi_set_param(uint32_t i, float v) {                                           \
        _UmiAdapter::set_param(i, v);                                                                    \
    }                                                                                                    \
    UMIM_EXPORT_ATTR float umi_get_param(uint32_t i) {                                                   \
        return _UmiAdapter::get_param(i);                                                                \
    }                                                                                                    \
    UMIM_EXPORT_ATTR uint32_t umi_get_param_count() {                                                    \
        return _UmiAdapter::get_param_count();                                                           \
    }                                                                                                    \
    UMIM_EXPORT_ATTR const char* umi_get_param_name(uint32_t i) {                                        \
        return _UmiAdapter::get_param_name(i);                                                           \
    }                                                                                                    \
    UMIM_EXPORT_ATTR float umi_get_param_min(uint32_t i) {                                               \
        return _UmiAdapter::get_param_min(i);                                                            \
    }                                                                                                    \
    UMIM_EXPORT_ATTR float umi_get_param_max(uint32_t i) {                                               \
        return _UmiAdapter::get_param_max(i);                                                            \
    }                                                                                                    \
    UMIM_EXPORT_ATTR float umi_get_param_default(uint32_t i) {                                           \
        return _UmiAdapter::get_param_default(i);                                                        \
    }                                                                                                    \
    UMIM_EXPORT_ATTR uint8_t umi_get_param_curve(uint32_t i) {                                           \
        return _UmiAdapter::get_param_curve(i);                                                          \
    }                                                                                                    \
    UMIM_EXPORT_ATTR uint16_t umi_get_param_id(uint32_t i) {                                             \
        return _UmiAdapter::get_param_id(i);                                                             \
    }                                                                                                    \
    UMIM_EXPORT_ATTR const char* umi_get_param_unit(uint32_t i) {                                        \
        return _UmiAdapter::get_param_unit(i);                                                           \
    }                                                                                                    \
...
#define UMIM_EXPORT_NAMED(ProcessorType, params_array, processor_name) \
    ...
    extern "C" {                                                       \
    UMIM_EXPORT_ATTR const char* umi_get_processor_name() {            \
