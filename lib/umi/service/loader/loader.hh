...
struct AppRuntime {
    ...
    [[nodiscard]] bool has_processor() const noexcept { return processor != nullptr && process_fn != nullptr; }
...
class AppLoader {
  public:
    ...
    void set_shared_memory(SharedMemory* shared) noexcept { shared_ = shared; }
    ...
    void call_process(std::span<float> output,
    ...
    [[nodiscard]] bool is_loaded() const noexcept { return runtime_.state != AppState::NONE; }

    /// Check if processor is available
    [[nodiscard]] bool has_processor() const noexcept { return runtime_.has_processor(); }
...
static_assert(sizeof(SharedMemory) <= 16 * 1024, "SharedMemory exceeds 16KB SHARED region");
