...
// umiboot Test Framework - Minimal standalone test framework
#pragma once
...
#define SECTION(name) umiboot::test::section(name)

#define TEST(name) static bool test_##name()

#define RUN_TEST(name) do { \
    if (test_##name()) { \
        umiboot::test::pass(#name); \
    } \
} while(0)

#define ASSERT(cond) do { \
    if (!(cond)) { \
        umiboot::test::fail(__func__, __FILE__, __LINE__, #cond); \
        return false; \
    } \
} while(0)
...
#define ASSERT_EQ(a, b) do { \
    if (!((a) == (b))) { \
        umiboot::test::fail(__func__, __FILE__, __LINE__, #a " == " #b); \
        return false; \
    } \
} while(0)
