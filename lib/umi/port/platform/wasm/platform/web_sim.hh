...
#include <umi/kernel/shell_commands.hh>
...
enum class TaskState : uint8_t { READY = 0, RUNNING, BLOCKED, SUSPENDED };
...
struct IrqFlagState {
    ...
    bool test(IrqFlag flag) const { return pending[static_cast<uint8_t>(flag)]; }

    uint32_t get_count(IrqFlag flag) const { return count[static_cast<uint8_t>(flag)]; }

    void clear_all() { pending.fill(false); }
};
...
struct TaskNotification {
    ...
    void notify(TaskId task, uint32_t bits) { values[static_cast<uint8_t>(task)] |= bits; }
    ...
    void clear(TaskId task) { values[static_cast<uint8_t>(task)] = 0; }
};
...
struct SharedMemoryManager {
    ...
    int allocate(RegionId id, size_t size, bool privileged = false) {
        ...
        if (pool_used + size > memory_pool.size())
            return -3; // OOM
    ...
    void* get(RegionId id, bool privileged_access = false) {
        ...
        if (!r.valid)
            return nullptr;
        if (r.privileged_only && !privileged_access)
            return nullptr;
        return r.base;
    }
    ...
    size_t get_size(RegionId id) const {
        if (id >= MAX_REGIONS || !regions[id].valid)
            return 0;
...
inline SysCallResult syscall(SysCall call, uint32_t arg0 = 0, uint32_t arg1 = 0, uint32_t arg2 = 0) {
    ...
    case SysCall::SHM_ALLOC:
        result.ret = g_shared_mem.allocate(static_cast<SharedMemoryManager::RegionId>(arg0), arg1, arg2 != 0);
        break;

    case SysCall::SHM_GET:
        result.ptr = g_shared_mem.get(static_cast<SharedMemoryManager::RegionId>(arg0), arg1 != 0);
    ...
    case SysCall::NOTIFY_WAIT:
        result.ret = static_cast<int32_t>(g_task_notify.wait(static_cast<TaskId>(arg0), arg1, arg2 != 0));
        break;
...
struct MemoryLayout {
    ...
    void update_warning() {
        ...
        uint32_t heap_free_percent = (heap.size > 0) ? ((heap.size - heap.used) * 100 / heap.size) : 100;
        uint32_t stack_free_percent =
            (task_stacks.size > 0) ? ((task_stacks.size - task_stacks.used) * 100 / task_stacks.size) : 100;
    ...
    void recalculate(uint32_t sram_kb,
                     uint32_t data_bss_kb,
                     uint32_t heap_kb,
                     uint32_t main_stack_kb,
                     uint32_t task_stack_bytes,
                     uint32_t task_count,
                     uint32_t dma_buffer_kb,
        ...
        // Calculate totals
        sram_used = data_bss.size + heap.used + task_stacks.used + main_stack.used + dma_buffers.size + shared_mem.size;
...
struct HwSimParams {
    ...
    float cycles_to_us(uint32_t cycles) const { return static_cast<float>(cycles) / static_cast<float>(cpu_freq_mhz); }

    // Get total heap in bytes
    uint32_t heap_total_bytes() const { return heap_size_kb * 1024; }

    // Get total stack in bytes (main stack only)
    uint32_t stack_total_bytes() const { return main_stack_kb * 1024; }

    // Get total task stacks in bytes
    uint32_t task_stacks_total_bytes() const { return task_stack_bytes * task_count; }
    ...
    bool is_valid() const { return sram_allocated_bytes() <= sram_total_kb * 1024; }

    // Get free SRAM (gap between heap and stacks)
    uint32_t sram_free_bytes() const {
        uint32_t allocated = sram_allocated_bytes();
        return (allocated < sram_total_kb * 1024) ? (sram_total_kb * 1024 - allocated) : 0;
    }

    // Recalculate memory layout
    void recalculate_layout() {
        g_memory_layout.recalculate(sram_total_kb,
                                    data_bss_kb,
                                    heap_size_kb,
                                    main_stack_kb,
                                    task_stack_bytes,
...
struct SysExLogBuffer {
    ...
    void write_stdout(const char* data, size_t len) { write(CMD_STDOUT, data, len); }

    void write_stderr(const char* data, size_t len) { write(CMD_STDERR, data, len); }

    void write_stdout(const char* str) { write_stdout(str, strlen(str)); }

    void write_stderr(const char* str) { write_stderr(str, strlen(str)); }
...
struct WebSimStateProvider {
    ...
    umi::os::ShellConfig& config() { return g_shell_config; }

    /// Get error log
    umi::os::ErrorLog<16>& error_log() { return g_error_log; }

    /// Get system mode (direct reference to global)
    umi::os::SystemMode& system_mode() { return g_system_mode; }
    ...
    /// Feed watchdog
    void feed_watchdog() { g_kernel_state.watchdog_last_feed_us = g_kernel_state.uptime_us; }
...
struct WebHwImpl {
    ...
    // --- Timer ---
    static void set_timer_absolute(uint64_t target_us) { timer_target_us_ = target_us; }

    static uint64_t monotonic_time_usecs() { return current_time_us_; }

    // Advance simulated time (called from process loop)
    static void advance_time(uint64_t delta_us) { current_time_us_ += delta_us; }
    ...
    static void watchdog_feed() { g_kernel_state.watchdog_last_feed_us = current_time_us_; }
    ...
    static uint32_t cycle_count() { return static_cast<uint32_t>(current_time_us_); }
    ...
    static void init_shared_memory() {
        ...
        syscall(SysCall::SHM_ALLOC, SharedMemoryManager::REGION_HW_STATE, sizeof(HwStateShared),
                1); // privileged
        hw_state_ptr_ = g_shared_mem.get_as<HwStateShared>(SharedMemoryManager::REGION_HW_STATE, true);
    ...
    static const HwStateShared* get_hw_state() { return hw_state_ptr_; }
    ...
    static void set_active_voices(uint32_t count) { active_voices_ = count; }

    // Add event processing cost
    static void add_event_cost(uint32_t count = 1) { active_events_ += count; }
    ...
    static void end_process(uint32_t budget_us, uint32_t buffer_size) {
        ...
        uint64_t total_cycles = static_cast<uint64_t>(hw.isr_overhead_cycles) +
        ...
        if (budget_us > 0) {
            load_x100 = static_cast<uint32_t>((static_cast<uint64_t>(simulated_process_us_) * 10000) / budget_us);
        }
    ...
    static void tick_driver_server() {
        ...
        if (hw_state_ptr_) {
            ...
            hw_state_ptr_->battery_flags =
                (g_kernel_state.battery_charging ? 0x01 : 0) | (g_kernel_state.battery_percent < 20 ? 0x02 : 0);
        }
    ...
    static uint32_t get_simulated_process_us() { return simulated_process_us_; }
    ...
    // --- RTC ---
    static void set_rtc(uint64_t epoch_sec) { g_kernel_state.rtc_epoch_sec = epoch_sec; }
    static uint64_t get_rtc() { return g_kernel_state.rtc_epoch_sec + (current_time_us_ / 1000000); }
};
...
#define UMI_WEB_SIM_EXPORT_KERNEL()                                                                            \
    ...
    UMI_WEB_EXPORT uint8_t umi_irq_flag_pending(uint8_t flag) {                                                \
        if (flag >= static_cast<uint8_t>(umi::web::IrqFlag::COUNT))                                            \
            return 0;                                                                                          \
    ...
    UMI_WEB_EXPORT uint32_t umi_notify_pending(uint8_t task) {                                                 \
        if (task >= static_cast<uint8_t>(umi::web::TaskId::COUNT))                                             \
            return 0;                                                                                          \
    ...
    UMI_WEB_EXPORT int32_t umi_shm_allocate(uint8_t region_id, uint32_t size, uint8_t privileged) {            \
        auto result = umi::web::syscall(umi::web::SysCall::SHM_ALLOC, region_id, size, privileged);            \
        return result.ret;                                                                                     \
    }                                                                                                          \
    UMI_WEB_EXPORT uint32_t umi_shm_size(uint8_t region_id) {                                                  \
        return static_cast<uint32_t>(                                                                          \
            umi::web::g_shared_mem.get_size(static_cast<umi::web::SharedMemoryManager::RegionId>(region_id))); \
    }                                                                                                          \
