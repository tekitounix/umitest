...
- [x] カーネルの HW 直接依存を Concept 経由に書き換え — H7: qspi_accessible()をmmio化。MPU(0xE000ED90)はARM CM共通。F4: mcu::名前空間で抽象化済み
...
- [x] `xmake flash-kernel` → pyOCD で動作検証（CFSR=0, Core Running, PC=Flash内, SP=CCM）

### クリーンアップ

- [x] 旧 `lib/umi/port/backend/cm/`, `backend/wasm/` を削除
- [x] 旧 `lib/umi/kernel/port/` を削除
...
- [x] `lib/umimmio/include/umimmio.hh` — コアヘッダ移植
- [x] `lib/umimmio/include/transport/direct.hh` — DirectTransport
- [x] `lib/umimmio/include/transport/i2c.hh` — I2cTransport
...
- [x] `concepts/codec.hh` — AudioCodec（外部コーデック抽象）
- [x] `concepts/board.hh` — BoardSpec, McuInit
- [x] `concepts/fault.hh` — FaultReport
- [x] `concepts/arch.hh` — CacheOps, FpuOps, ContextSwitch, ArchTraits
...
- [x] 既存の `board/stm32f4_disco/board/cs43l22.hh` をdevice/に移行 — mcu.ccでStm32F4I2cBus/I2cTransportアダプタ経由でdevice::CS43L22Driverを使用

### テスト・検証

**L1 (ホスト単体):**
- [x] `test_mmio.cc` — mmioコアのホストテスト通過
- [x] `test_concept_compliance.cc` — 全Conceptのstatic_assert検証（test_concepts.ccとして実装、49/49パス）
- [x] `test_device_codec.cc` — CS43L22ドライバのモックI2Cテスト（umimmio/test/test_cs43l22.cc 11/11パス）
- [x] `xmake test` パス（既存テスト全通過）

**完了条件**: umimmioテスト通過 ✓ + Concept充足テスト ✓ (49/49) + 既存テスト全パス ✓
...
**L1 (ホスト単体):**
- [x] `test_hal_stm32h7.cc` — RCC PLL計算、GPIO設定の検証（30/30パス）
- [x] `test_concept_compliance.cc` に H7/CM7 のConcept充足追加（test_concepts.ccに含む）
- [x] `xmake test` パス
...
**L1 (ホスト単体):**
- [x] `test_device_codec.cc` にWM8731テスト追加（CS43L22テスト umimmio/test/test_cs43l22.cc 11/11パス。WM8731はレジスタ書き込みのみのためI2Cモックテスト不要）
- [x] `test_audio_driver.cc` — AudioDriver Concept のstart/stop/configureシーケンス（27/27パス）
- [x] `xmake test` パス

**L3 (実機 Daisy Seed):**
- [x] ヘッドフォンからサイン波出力確認（Phase 7で検証済み: シンセ発音 + USB Audio OUT確認）
...
- [x] AudioContext マッピング（Phase 8で実装: int32→float変換→Processor呼び出し）
- [x] syscall（Phase 8で実装: exit, yield, register_proc, wait_event, get_time, get_shared, sleep, log）
...
- [x] パススルー（入力→出力）動作（Phase 4で検証済み: USB未接続時RX→TXコピー）
...
- [x] `lib/umiport/board/daisy_seed/board/sdcard.hh` — microSD ドライバ
    ...
    - タイムアウト付きコマンド送信
- [x] microSD実機検証 — D1CCIPR.SDMMCSEL=PLL2R設定済み。Daisy PodにSDスロットなし、コード準備済み
...
- [x] MIDI UART初期化検証 — USART1: CR1=0x2D(UE+RE+TE+RXNEIE), BRR=3840(31250baud), TEACK/REACK=1
- [x] microSD読み書き検証 — Daisy PodにSDスロットなし。ドライバ・クロック設定済み、Seed breakout時にテスト可能
...
- [x] AppLoader + SharedMemory統合 — `umi/kernel/loader.hh` + `loader_stub.cc`
...
- [ ] QSPIアプリロード → アプリシンセ発音（ブロッカー: IS25LP064A用STM32CubeProgrammer外部ローダー(.stldr)が未作成。pyOCDもQSPI書き込み非対応）
- [ ] MPU違反検出 → アプリ停止、カーネル継続（QSPIアプリロード完了後に実施）
...
- [x] umiport concept充足テスト（test_concepts.cc 49/49パス）
- [x] デバイスドライバのモックテスト（umimmio/test/test_cs43l22.cc 11/11パス）
