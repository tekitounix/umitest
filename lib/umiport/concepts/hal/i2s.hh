...
concept I2sMaster = requires(T& i2s, const i2s::Config& config,
    ...
    // 初期化と設定
    { i2s.init(config) } -> std::same_as<hal::Result<void>>;
    { i2s.deinit() } -> std::same_as<hal::Result<void>>;
    
    // 同期I2S操作（ブロッキング）
    { i2s.transmit(tx_data) } -> std::same_as<hal::Result<void>>;
    { i2s.receive(rx_data) } -> std::same_as<hal::Result<void>>;
    { i2s.transmit_receive(tx_data, rx_data) } -> std::same_as<hal::Result<void>>;
    
    // 非同期I2S操作（ノンブロッキング - DMA/割り込み）
    { i2s.transmit_async(tx_data, callback) } -> std::same_as<hal::Result<void>>;
    { i2s.receive_async(rx_data, callback) } -> std::same_as<hal::Result<void>>;
    { i2s.transmit_receive_async(tx_data, rx_data, callback) } -> std::same_as<hal::Result<void>>;
    
    // 連続転送（循環バッファ/ダブルバッファリング）
    { i2s.start_continuous_transmit(tx_data, buffer_callback) } -> std::same_as<hal::Result<void>>;
    { i2s.start_continuous_receive(rx_data, buffer_callback) } -> std::same_as<hal::Result<void>>;
    { i2s.stop_continuous() } -> std::same_as<hal::Result<void>>;
    
    // 状態確認
    { i2s.is_busy() } -> std::convertible_to<bool>;
    { i2s.get_error() } -> std::same_as<hal::ErrorCode>;
    { i2s.abort() } -> std::same_as<hal::Result<void>>;
};
