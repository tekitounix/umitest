...
concept InterruptController = requires(T& intc, 
    ...
    // ハンドラ登録（必須）
    { intc.register_handler(irq_number, handler) } -> std::same_as<hal::Result<void>>;
    
    // 割り込み制御（必須）
    { intc.enable_irq(irq_number) } -> std::same_as<hal::Result<void>>;
    { intc.disable_irq(irq_number) } -> std::same_as<hal::Result<void>>;
    
    // 優先度設定（オプション - NOT_SUPPORTEDを返してもよい）
    { intc.set_priority(irq_number, priority) } -> std::same_as<hal::Result<void>>;
    ...
    // ペンディングクリア（オプション）
    { intc.clear_pending(irq_number) } -> std::same_as<hal::Result<void>>;
};
