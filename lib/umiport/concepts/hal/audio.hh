...
// オーディオデバイスインターフェース
template <typename T>
concept AudioDevice = requires(T& device, const hal::audio::Config& config) {
    // 設定と初期化
    { device.configure(config) } -> std::same_as<hal::Result<void>>;
    { device.get_config() } -> std::same_as<hal::audio::Config>;
    { device.is_config_supported(config) } -> std::convertible_to<bool>;
    
    // 状態管理
    { device.start() } -> std::same_as<hal::Result<void>>;
    { device.stop() } -> std::same_as<hal::Result<void>>;
    { device.pause() } -> std::same_as<hal::Result<void>>;
    { device.resume() } -> std::same_as<hal::Result<void>>;
    { device.get_state() } -> std::same_as<hal::audio::State>;
...
concept CallbackAudioDevice = AudioDevice<T> && requires(T& device, hal::audio::Callback<SampleType> callback) {
    { device.set_callback(callback) } -> std::same_as<hal::Result<void>>;
};

// ブロッキングI/Oオーディオデバイス
template <typename T, typename SampleType>
concept BlockingAudioDevice = AudioDevice<T> && requires(T& device, std::span<SampleType> buffer) {
    { device.write(buffer) } -> std::same_as<hal::Result<std::size_t>>;  // 書き込んだフレーム数
    { device.read(buffer) } -> std::same_as<hal::Result<std::size_t>>;   // 読み込んだフレーム数
};
