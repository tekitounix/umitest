...
| 8 | アプリケーション完全分離 | 未着手 | syscall/MPU/リンカ変更 |
...
- [ ] `lib/bsp/`, `lib/hal/` のレガシーパスを削除（umiport board/mcu/ 移行後）
...
- [ ] D-Cache再有効化 — MPU non-cacheable設定でもD-Cacheが書き込みをキャッシュする問題の根本解決が必要
...
- [ ] カーネルからシンセ実装を分離 — 現在main.ccに埋め込まれたrender_synth/voicesをアプリに移動
- [ ] Processor登録syscall実装 — `RegisterProc` syscallでアプリがProcessorを登録
- [ ] AudioContext構築 — DMA ISR→float変換→AudioContext構築→Processor呼び出し→int32変換→DMA TX
- [ ] アプリローダー — .umiaバイナリをQSPIまたはSDRAMからロード、MPU設定
- [ ] 共有メモリ（`_shared_start`） — OS/App間のバッファ交換領域をリンカスクリプトに定義
- [ ] syscallハンドラ拡張 — WaitEvent, GetTime, Sleep, MidiSend/Recv, GetShared等

### アプリ側

- [ ] `examples/synth_app/` をH7対応 — 既存F4向けsynth_appのH7ビルド
- [ ] .umiaバイナリ生成 — xmake.luaにアプリターゲット追加
- [ ] `main()` → Controller task — 起動時にProcessor登録、以降はイベントループ

### リンカスクリプト変更

- [ ] カーネル用: アプリ領域をリザーブ（QSPI or SDRAM）
- [ ] アプリ用: 独立リンカスクリプト、エントリポイント、共有メモリ参照

### テスト・検証

**L1 (ホスト単体):**
- [ ] syscallディスパッチのユニットテスト
- [ ] AudioContext構築→Processor呼び出しのテスト

**L3 (実機):**
- [ ] カーネル単体起動（アプリなし）→ パススルーまたは無音
- [ ] アプリロード → シンセ発音
...
- [ ] D2 SRAM DMAバッファのコヒーレンシ解決
    - 選択肢A: DMA前後にDCIMVAC/DCCMVACを正しく適用（現在のコードにはあるがD-Cache無効）
    - 選択肢B: D2 SRAM全体をMPU Strongly-Ordered属性に設定
    - 選択肢C: DMAバッファのみDTCMに移動（DTCMはD-Cache非経由）
- [ ] D-Cache有効化後の全機能リグレッションテスト

### デバッグ計測クリーンアップ

- [ ] d2_dbg[16]を削除またはコンパイルスイッチ化
- [ ] HardFault_Handler: リリース時はLED点灯のみに簡素化
