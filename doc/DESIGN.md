...
| 構成 | 用途 | 特徴 |
|------|------|------|
| **マイクロカーネル** | 製品カスタムファームウェア | ユーザーはハード制御不可。MPU対応MCUでは保護を有効化 |
| **モノリシック** | 開発ボード | ユーザーがハード制御。全機能が単一アドレス空間 |

どちらの構成でも基本的に同じアプリケーションコードが動作する。
...
- **ユーザーが使えるタスク**: Audio Task (Realtime)、User Task のみ
- **ハード制御**: カーネル側のServer Taskが担当
- **データ交換**:
  - **速度重視**: 共有メモリ経由で直接アクセス（オーディオバッファ等）
  - **安全性重視**: システムコール経由（GPIO設定、パラメータ変更等）
