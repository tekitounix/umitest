...
ISR → Task 通知は `notify()` / `wait_block()` で実現する。
...
ISR またはタスクからイベントフラグを設定する。対象タスクが Blocked 状態かつマスクが一致すれば Ready に遷移させる。
...
```
優先度 5:  Audio DMA (I2S, PDM) — 最高。クリティカルセクション中も実行可能
優先度 6:  USB OTG FS
優先度 F0: SysTick
優先度 FF: PendSV — 最低（コンテキストスイッチ用）
```

ISR は最小限の処理（フラグ設定 + notify）のみ行い、実際の処理はタスクで行う。
