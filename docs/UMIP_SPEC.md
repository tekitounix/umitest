...
UMIPはUMIアーキテクチャの最も基本的な部分を担い、以下の設計思想を実現します。
...
- **組み込み** - ハードウェアシンセ、エフェクター
- **ネイティブ** - VST3/CLAP/AUプラグイン、スタンドアロン
...
| 観点 | VST3/AU/CLAP | UMI |
|------|-------------|------|
| 対象環境 | デスクトップのみ | **組み込み〜ブラウザ** |
| UIの位置づけ | UIありが一般的 | **ヘッドレスがデフォルト** |
| 設計基準 | DAW環境 | **組み込み（低リソース）** |
| UI実装 | プラットフォーム固有 | **WASM/WebView** |

※ VST3/AU/CLAPも仕様上はUI分離可能だが、実運用ではUIありが一般的
※ UMIはDSPコードが同一ソース、UIはWASM/WebViewで同一バイナリ化可能

### UIアーキテクチャ

UIは**WASM**または**HTML/CSS/JS（WebView）**で実装:

- **デスクトップ/ブラウザ**: WebViewでそのまま動作
- **組み込み**: UIシミュレータで開発・テスト
- **EC/デモサイト**: 製品ページで実動デモが可能

DSPとUIが完全分離されているため:
- 同一UIコードがプラグイン・スタンドアロン・Webで動作
- 組み込み向けUIは別途最適化可能（または汎用UIをホストが提供）

### なぜヘッドレスが重要か

1. **テスト容易性** - 入力→出力の純粋関数に近く、CI自動化が容易
2. **ポータビリティ** - UI層を切り離すことで環境依存を排除
3. **関心の分離** - DSP（安定）とUI（環境依存）を明確に分離
...
- DSP処理のみに専念
- パラメータはメンバ変数として公開
- UI状態管理、イベントルーティング、入力処理はホスト/カーネルに委譲

### モードレスとの違い

| 用語 | 定義 |
|------|------|
| **モードレス** | 操作モード（ページ切替、選択状態）を持たない |
| **ヘッドレス** | UIロジック自体を持たない（UMIC不要） |

モードレスはヘッドレスの必要条件だが、ヘッドレスはさらにUI処理全体を外部化する。
...
| MIDI Learn（モジュール内管理） | ✗ | マッピング状態あり → UMIC必要 |
