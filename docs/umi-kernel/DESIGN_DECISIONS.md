...
```
arm-none-eabi-gcc (-fPIE -pie) → ELF → elf2umia → .umia (relocatable ELF)
```

`elf2umia`はELFからumia形式への変換ツール。xmakeビルドルールから自動呼び出し。
...
```
OS: elf2flash(slot, app.umia)  ← リロケーション + Flash書き込み
...
umiaの既存リロケーションロジックを再利用できるため、実装コストも低い。
