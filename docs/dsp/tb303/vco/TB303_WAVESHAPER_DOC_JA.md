...
|---------------|-----|------|
| R1 | 10kΩ | GNDへの入力負荷 |
| R2 | 100kΩ | 直接パス: 入力 → ベース |
| R3 | 10kΩ | コンデンサパス: C1 → ベース |
| R4 | 22kΩ | +12Vへのエミッタ抵抗 |
| R5 | 10kΩ | +5.33Vへのコレクタ抵抗 |
| C1 | 10nF | メモリコンデンサ（ハイシェルフフィルタ） |
| C2 | 1µF | エミッタバイパスコンデンサ |
...
1. **v_cap節点**: `i_C1 = i_R3`
2. **v_base節点**: `i_R2 + i_R3 + i_B = 0`
3. **v_emitter節点**: `i_R4 - i_E - i_C2 = 0`
4. **v_collector節点**: `i_R5 + i_C = 0`
