...
class SimpleSynth {
  public:
    explicit SimpleSynth(std::uint32_t sample_rate) : sample_rate_(static_cast<float>(sample_rate)) {
        update_phase_inc();
    }
    ...
    // === HasParams: optional ===
    std::span<const umi::ParamDescriptor> params() const { return params_; }
  ...
  private:
    ...
    void update_phase_inc() { phase_inc_ = frequency_ * two_pi_ / sample_rate_; }
...
extern "C" void
umi_audio_process(float* input, float* output, std::size_t frames, std::uint8_t channels, std::uint32_t sample_rate) {
    (void)input;
    ...
    umi::AudioContext ctx{.inputs = std::span<const umi::sample_t* const>(inputs_arr, 0), // 0 inputs
